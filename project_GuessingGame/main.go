...
func main() {
	...
	for i := range chutes {
		...
		if err != nil {
			fmt.Println("O seu chute tem ser um número inteiro")
			return
		}
		switch {
		case chuteInt < x:
			fmt.Println("Você errou. O número sorteador é maior que:", chuteInt)
		case chuteInt > x:
			fmt.Println("Você errou. O número sorteador é menor que:", chuteInt)
		case chuteInt == x:
